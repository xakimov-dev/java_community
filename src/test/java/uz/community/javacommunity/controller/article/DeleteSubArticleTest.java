...
class DeleteSubArticleTest extends CommonIntegrationTest {
    ...
    @Test
    @DisplayName(value = "Should fail if token is invalid")
    void shouldFailInvalidToken() throws Exception {
        //GIVEN
        ...
        SubArticleResponse subArticle = testDataHelperSubArticle.createSubArticle(category.getId(),
                article.getArticleId(), null, "sub_article");
        RequestBuilder request = delete("/article/sub/delete/" + subArticle.id())
                .header("Authorization", "Bearer " + token); // set the Authorization header with the generated token
        //WHEN
        ResultActions resultActions = mockMvc.perform(request);
